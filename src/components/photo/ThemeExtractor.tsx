...
export function ThemeExtractor({
  ...
  const [draggingMarkerIdx, setDraggingMarkerIdx] = useState<number | null>(
    null,
  );
    ...
    (currentMarkers = markers) => {
      ...
      const newColors = currentMarkers.map((marker) => {
        const x = Math.max(
          ...
          Math.min(
            canvas.width - 1,
            Math.floor((marker.x / 100) * canvas.width),
          ),
        );
        const y = Math.max(
          0,
          Math.min(
            canvas.height - 1,
            Math.floor((marker.y / 100) * canvas.height),
          ),
        );
        try {
  ...
  const applyMood = useCallback((hex: string, currentMood: Mood): string => {
    ...
    switch (currentMood) {
      case "colorful":
        return hslToHex({ ...hsl, s: Math.min(100, hsl.s + 30) });
      case "bright":
        return hslToHex({ ...hsl, l: Math.min(100, hsl.l + 30) });
      case "muted":
        return hslToHex({ ...hsl, s: Math.max(0, hsl.s - 40) });
      case "deep":
        return hslToHex({
          ...hsl,
      ...
      case "dark":
        return hslToHex({ ...hsl, l: Math.max(0, hsl.l - 40) });
      default:
        return hex;
    }
  }, []);
      ...
      <div className="lg:col-span-3 space-y-8">
        {!image ? (
          ...
          >
            <Upload
              size={48}
              className="text-text-muted mb-4 group-hover:scale-110 transition-transform"
            />
                ...
                <div
                  ...
                  className={cn(
                    ...
                    draggingMarkerIdx === idx &&
                      "scale-125 ring-4 ring-accent-primary/50",
                  )}
                  style={{
                    left: `${marker.x}%`,
                    top: `${marker.y}%`,
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const wheelTab = Array.from(tabs).find((b) =>
                      b.textContent?.includes("Color Wheel"),
                    );
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                {processedColors.length > 0
                  ? processedColors.map((hex, idx) => (
                      <CardGlass
                        key={idx}
                        className="p-4 flex flex-col gap-4 group hover:scale-105 transition-all duration-medium"
                      >
                        <div
                  ...
                  : Array.from({ length: 6 }).map((_, i) => (
                      <div
                        key={i}
                        className="aspect-square w-full rounded-2xl bg-white/5 animate-pulse"
                      />
                    ))}
              </div>
            </div>
